...
class Meet(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Auto-determine and get/create session from date
        if self.date and not self.session:
    ...
    def __str__(self):
        ...
        return f"Rencontre #{self.pk}"


class Problem(models.Model):
    # Proper auto pk — link is unique but not the pk so admin M2M works correctly
    ...
    def get_difficulty(self):
        """Get difficulty, fetching if not cached"""
        if self.difficulty:
